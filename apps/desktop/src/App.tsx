...
import {
  AlertCircle, Bold, Bot, Check, ChevronDown, ClipboardPaste, Copy, Database, Download, ExternalLink,
...
export default function App() {
  ...
  const deleteSelection = useCallback(() => {
    ...
    if (nodeIds.length) notify(`已从画布移除 ${nodeIds.length} 个项目，电脑原文件保持不变${edgeIds.length ? `；同时删除 ${edgeIds.length} 条连接线` : ''}`);
  ...
  useEffect(() => {
    ...
    Promise.all([window.desktop.loadWorkspace(), window.desktop.listAssets()]).then(([workspace, assets]) => {
      if (!active) return;
      const pages = (workspace?.canvases?.length ? workspace.canvases : [{ id: 'main', title: '知源星图', nodes: workspace?.nodes || [], edges: workspace?.edges || [] }])
        .map((page) => ['哇塞-超级桌面', '哇塞·超级桌面'].includes(page.title) ? { ...page, title: '知源星图' } : page);
    ...
    }).catch(() => { setCanvasPages([{ id: 'main', title: '知源星图', nodes: [], edges: [] }]); store.hydrate(null, []); });
  ...
  const createDocument = (documentType: 'txt' | 'word') => {
    const position = contextMenu ? flow.screenToFlowPosition({ x: contextMenu.x, y: contextMenu.y }) : flow.screenToFlowPosition({ x: innerWidth / 2, y: innerHeight / 2 });
    const id = store.addNode({ kind: 'file', title: documentType === 'txt' ? '新建文本文档.txt' : '新建 Word 文档.docx', subtitle: '画布内部文档', documentType, documentContent: '', status: 'virtual' }, position);
...
function DocumentEditor({ nodeId, onClose, notify }: { nodeId: string; onClose: () => void; notify: (message: string) => void }) {
  ...
  const save = (close = true) => {
    ...
    updateNode(nodeId, { title: title.trim() || node.data.title, documentContent: html, body: plain, subtitle: '画布内部文档' });
    notify('文档已保存到画布，不会修改电脑文件'); if (close) onClose();
  };
  const exportToComputer = async () => {
    const html = isWord ? (editorRef.current?.innerHTML || content) : content;
    const plain = isWord ? (editorRef.current?.innerText || '') : content;
    const nextTitle = title.trim() || node.data.title;
    updateNode(nodeId, { title: nextTitle, documentContent: html, body: plain, subtitle: '画布内部文档' });
    try {
      const result = await window.desktop.exportCanvasDocument({ type: isWord ? 'word' : 'txt', title: nextTitle, content: html });
      if (!result.canceled && result.path) notify(`已保存到电脑：${result.path}`);
    } catch (error) { notify(error instanceof Error ? error.message : '保存到电脑失败'); }
  };
  ...
  return <div className="modal-layer"><section className="document-editor-modal"><header><div><span><FileText size={18}/></span><div><input value={title} onChange={(event) => setTitle(event.target.value)} aria-label="文档名称"/><small>画布内部{isWord ? ' Word' : ' TXT'} 文档 · 可导出副本到电脑</small></div></div><button onClick={() => save()}><X size={17}/></button></header>{isWord && <nav><button onClick={() => format('bold')} title="加粗"><Bold size={15}/></button><button onClick={() => format('italic')} title="斜体"><Italic size={15}/></button><button onClick={() => format('insertUnorderedList')} title="项目符号"><List size={15}/></button><span>选中文字后使用格式工具</span></nav>}<div className="document-editor-body">{isWord ? <div ref={editorRef} className="word-editable" contentEditable suppressContentEditableWarning onInput={(event) => setContent(event.currentTarget.innerHTML)} data-placeholder="开始输入文档内容…"/> : <textarea autoFocus value={content} onChange={(event) => setContent(event.target.value)} placeholder="开始输入文本内容…"/>}</div><footer><span>⌘S 保存到画布 · 导出会在电脑上生成独立副本</span><button onClick={() => save(false)}><Save size={14}/>保存到画布</button><button onClick={() => void exportToComputer()}><Download size={14}/>保存到电脑</button><button className="primary" onClick={() => save()}><Check size={14}/>完成</button></footer></section></div>;
}
...
function CanvasVideoPlayer({ asset, onClose }: { asset?: Asset; onClose: () => void }) {
  ...
  return <section className="canvas-video-player nodrag nopan" onClick={(event) => event.stopPropagation()}><header><div><Play size={15}/><strong>{asset.name}</strong></div><button onClick={onClose} title="关闭视频"><X size={16}/></button></header><video src={`sourceatlas-media://asset/${encodeURIComponent(asset.id)}`} controls autoPlay playsInline/><footer><span>视频在主画布上方播放，不会跳出应用</span><button onClick={() => void window.desktop.revealAsset(asset.path)}><ExternalLink size={12}/>在 Finder 中显示</button></footer></section>;
}
...
function FolderCanvas({ assetId, nodeId, onClose, onImport, onCopy, onPaste, canPaste, notify }: { assetId?: string; nodeId?: string; onClose: () => void; onImport: () => void; onCopy: () => boolean; onPaste: () => void; canPaste: boolean; notify: (message: string) => void }) {
  ...
  const createInnerDocument = (type: 'txt' | 'word') => createInnerNode({ kind: 'file', title: type === 'txt' ? '未命名文本.txt' : '未命名文档.docx', subtitle: '画布内部文档', documentType: type, documentContent: '', body: '', status: 'ready' });
  ...
  const deleteInnerSelection = () => { const ids = currentInnerSelection(); if (ids.length) { removeNodes(ids); notify(`已从画布移除 ${ids.length} 个项目，电脑原文件保持不变`); } setInnerMenu(null); };
