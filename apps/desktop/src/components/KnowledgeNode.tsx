...
const DESKTOP_FOLDER_ICON = new URL('./macos-folder.png', document.baseURI).href;
const DESKTOP_DOCUMENT_ICON = new URL('./macos-document.png', document.baseURI).href;
...
function KnowledgeNodeComponent({ id, data, selected }: NodeProps<CanvasNode>) {
  ...
  const isImportedAsset = Boolean(asset);
  const isCanvasDocument = Boolean(data.documentType && !asset);
  const sourceSubtitle = isImportedAsset
    ? asset?.kind === 'folder' ? '本地原文件夹 · 路径引用' : '本地原文件 · 路径引用'
    : isCanvasDocument ? '画布内部文档 · 非电脑文件' : data.subtitle || data.kind;
  ...
  return <article style={{ '--connection-handle-size': `${connectionHandleSize}px` } as CSSProperties} className={`knowledge-node kind-${data.kind} ${data.kind === 'knowledge-base' && answerText ? 'has-kb-answer' : ''} ${isDesktopAsset ? 'desktop-asset-node' : ''} ${isImportedAsset ? 'source-local-file' : ''} ${isCanvasDocument ? 'source-canvas-document' : ''} ${selected ? 'is-selected' : ''} ${locatedId === id ? 'is-located' : ''}`} onDoubleClick={openNode}>
    ...
    <header><span className={`node-kind-icon ${(nativeIcon || asset || ['file','folder'].includes(data.kind)) ? 'native' : ''} ${isQuickLookDocument ? 'quicklook-document' : ''} ${isVideoPreview ? 'quicklook-video' : ''}`}>{isVideoPreview && !videoFailed ? <video key={asset!.id} src={`sourceatlas-media://asset/${asset!.id}`} autoPlay loop muted playsInline preload="auto" draggable={false} onError={() => setVideoFailed(true)} aria-label={`${data.title}视频预览`}/> : nativeIcon ? <img src={nativeIcon} alt=""/> : (asset || ['file','folder'].includes(data.kind)) ? <span className="macos-file-icon"><img src={(asset?.kind || data.kind) === 'folder' ? DESKTOP_FOLDER_ICON : DESKTOP_DOCUMENT_ICON} alt=""/>{(asset?.kind || data.kind) === 'file' && <b>{asset?.extension.slice(1, 5).toUpperCase() || 'FILE'}</b>}</span> : <Icon size={18} />}</span><div><small>{sourceSubtitle}</small>{editingTitle ? <input className="node-title-input nodrag nopan" autoFocus value={titleDraft} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} onChange={(event) => setTitleDraft(event.target.value)} onBlur={commitTitle} onDoubleClick={(event) => event.stopPropagation()} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); commitTitle(); } if (event.key === 'Escape') { setTitleDraft(data.title); setEditingTitle(false); } }}/>:<strong className="node-title nodrag nopan" title={isImportedAsset ? '双击修改画布显示名称，不会重命名电脑原文件' : '双击编辑名称'} onPointerDown={(event) => event.stopPropagation()} onDoubleClickCapture={(event) => { event.preventDefault(); event.stopPropagation(); setTitleDraft(data.title); setEditingTitle(true); }}>{data.title}</strong>}</div>{Boolean(data.body) && ['ai', 'answer'].includes(data.kind) && <button className="node-content-copy nodrag nopan" title="一键复制全部内容" onClick={(event) => { event.stopPropagation(); void copyBody(); }}>{copied ? <Check size={13}/> : <Copy size={13}/>}</button>}{data.kind === 'ai' && <button className="node-prompt-button nodrag" title="设置该 AI 节点的系统提示词" onClick={() => { setPromptDraft(data.instructions || ''); setPromptOpen(!promptOpen); }}><Settings size={13}/></button>}{data.status && <em className={`node-status status-${data.status}`}>{statusText(data.status)}</em>}</header>
    {data.kind === 'knowledge-base' && <>
      <div className="kb-sources"><span><Folder size={12} />拖入文件资料</span><span><Sparkles size={12} />限定 AI 范围</span></div>
      <div className={`kb-drop-area ${(data.childNodeIds || []).length ? 'has-items' : ''}`}>{(data.childNodeIds || []).length ? (data.childNodeIds || []).map((childId) => { const child = nodes.find((node) => node.id === childId); if (!child) return null; const childAsset = child.data.assetId ? assets.find((item) => item.id === child.data.assetId) : undefined; const childSource = childAsset ? '本地原文件 · 路径引用' : child.data.documentType ? '画布内部文档 · 非电脑文件' : child.data.subtitle || '关联资料'; return <div className="kb-child-item nodrag nopan" key={childId} onDoubleClick={(event) => { event.stopPropagation(); if (child.data.documentType) window.dispatchEvent(new CustomEvent('open-canvas-document', { detail: { nodeId: childId } })); else if (childAsset?.path) void window.desktop.openAsset(childAsset.path); }}><ChildPreview asset={childAsset} kind={child.data.kind}/><div><strong>{child.data.title}</strong><small>{childSource}</small></div><button title="移出知识库（原文件保留）" onClick={(event) => { event.stopPropagation(); detachNodeFromKnowledge(id, childId); }}><X size={12}/></button></div>; }) : <div className="kb-drop-empty"><Folder size={20}/><strong>把文件、图片或文档拖到这里</strong><span>本地文件只建立路径引用，不会复制原文件</span></div>}</div>
    ...
    {(data.kind === 'file' || data.kind === 'folder') && <div className="asset-foot"><span>{asset?.kind === 'folder' ? `${asset.childCount || 0} 个项目` : asset?.extension.slice(1).toUpperCase() || (data.documentType === 'word' ? 'WORD' : data.documentType?.toUpperCase())}</span><span>{asset?.indexStatus ? `原文件 · ${statusText(asset.indexStatus)}` : data.documentType ? '仅存画布' : '画布对象'}</span></div>}
  </article>;
}
